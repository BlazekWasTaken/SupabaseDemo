...
//    setAccelerometer: (reading: Reading) -> Unit
    ...
    var acceleration by remember { mutableStateOf(Reading(0F, 0F, 0F)) }
    ...
    LaunchedEffect(Unit) {
        ...
        val sensorEventListener = object : SensorEventListener {
            override fun onSensorChanged(event: SensorEvent?) {
                if (event != null) {
                    acceleration = Reading(event.values[0], event.values[1], event.values[2])
...
//                    setAccelerometer(Reading(event.values[0], event.values[1], event.values[2]))
                    UwbManagerSingleton.updateAccelerometerReadings(acceleration)
                }
            }
